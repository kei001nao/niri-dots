...
PREFERRED_PLAYERS="spotify,mpv,vlc,firefox,chromium,brave,chrome,mpd"
...
select_player() {
    ...
    [[ -z "$all_players" ]] && echo "" && return

    # Define player priority. Will check for players in this order.
    local priority_order=("spotube" "spotify" "mpd" "mpv" "io.github.celluloid_player.Celluloid" "vlc" "firefox" "chromium" "brave" "chrome")
    
    # 1. Find a player that is currently "Playing", in priority order
    for p_base in "${priority_order[@]}"; do
        # Find the full name (e.g., spotube.instance123) that matches the base name
        local player_instance
        player_instance=$(echo "$all_players" | grep -E "^${p_base}" | head -n1)

        if [[ -n "$player_instance" ]]; then
            if [[ "$(playerctl -p "$player_instance" status 2>/dev/null)" == "Playing" ]]; then
                echo "$player_instance"
                return
            fi
        fi
    done

    # 2. If no player is "Playing", find one that is "Paused", in priority order
    for p_base in "${priority_order[@]}"; do
        local player_instance
        player_instance=$(echo "$all_players" | grep -E "^${p_base}" | head -n1)

        if [[ -n "$player_instance" ]]; then
            if [[ "$(playerctl -p "$player_instance" status 2>/dev/null)" == "Paused" ]]; then
                echo "$player_instance"
                return
            fi
        fi
    ...
    # 3. Fallback: return nothing if no player is playing or paused
    echo ""
}
...
get_filename_from_url() {
  ...
  if [[ "$url" =~ ^file:// ]]; then
    ...
    local path="${url#file://}"
    decoded_path=$(printf '%b' "${path//%/\\x}")
    filename="$(basename "$decoded_path")"
  elif [[ "$url" =~ ^https?:// ]]; then
    # For http/https URLs, just return the URL for now, or consider parsing further
    filename="$(basename "$url")"
  else
    # Treat as a direct path or other format
    filename="$(basename "$url")"
  fi
  printf '%s' "$filename"
}
...
--title)
  ...
  printf '%s\n' "$(trim_string "${title}" "$TITLE_MAX_LEN")"
  ;;

--artist)
  artist="$(get_metadata 'xesam:artist')"
  if [[ -n "$artist" ]]; then
    printf '󰠃 %s\n' "$(trim_string "$artist" "$ARTIST_MAX_LEN")"
  fi
  ;;
